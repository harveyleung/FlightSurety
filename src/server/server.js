    ...
    if (error)
    {
        ...
        for(let idx=0; idx < oracle_accounts.length; idx++) {
            ...
            if(oracle_accounts[idx].indexes.includes(index)) {
                console.log("Oracle response matches a request: " + JSON.stringify(oracle_accounts[idx]));
                ...
                flightSuretyApp.methods
                    ...
                    .send({ from: oracle_accounts[idx].address, gas: 200000 }, (error, result) => {
                        if(error) {
                            ...
                            console.log("Sent Oracle Response: " + JSON.stringify(oracle_accounts[idx]) + " with Status Code: " + statusCode);
                        }
                    });
            }
        }
